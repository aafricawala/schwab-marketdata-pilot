...
class MasterThesisCalculator:
  ...
  def __init__(
    ...
    self.q_class = str(
        self.raw.get("phase_0_grounding", {}).get("quote_age_classification")
        or ""
    )
  ...
  def _calc_step_1(self) -> Dict[str, Any]:
    ...
    if is_halted:
      ...
      calc["earnings_yield_state"] = "N/A"
      calc["earnings_yield_reason"] = (
          "warrant_derivative_instrument_no_corporate_earnings"
      )
    elif is_etn:
      calc["earnings_yield_state"] = "N/A"
      calc["earnings_yield_reason"] = (
    ...
    if is_halted:
      ...
      if payout is not None and payout > 100.0:
        calc["dividend_payout_ratio_health"] = (
            "UNSUSTAINABLE_COVERAGE_DEFICIT"
        )
  ...
  def _calc_step_3_7(self) -> Dict[str, Any]:
    ...
    if self.opt_df.empty or "putCallIndicator" not in self.opt_df.columns:
      return {
    ...
    flow: Dict[str, Any] = {
        ...
        "put_call_open_interest_ratio": (
            round(oir, 4) if oir is not None else None
        ),
  ...
  def _calc_cmi_30d(self, df: pd.DataFrame) -> Dict[str, Any]:
    if (
        df.empty
        or "daysToExpiration" not in df.columns
        or "strikePrice" not in df.columns
    ):
      return {
          "state": "UNKNOWN",
          "reason": "options_chain_empty_or_unavailable",
  ...
  def _calc_skew_30d(
    ...
    if (
        df.empty
        or "daysToExpiration" not in df.columns
        or "putCallIndicator" not in df.columns
    ):
      ret = {
          "state": "UNKNOWN",
    ...
    anchor_label = (
        "PRIMARY_25D_SYMMETRIC"
        if is_primary
        else "FALLBACK_NEAREST_SYMMETRIC"
    )
    s_regime = (
        "REVERSE_CALL_SKEW"
  ...
  def _calc_step_5(self) -> Dict[str, Any]:
    ...
    clean_bars = self.ph_df.dropna(
        ...
        "datetime" if "datetime" in self.ph_df.columns else self.ph_df.columns[0]
    )
