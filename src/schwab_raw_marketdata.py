...
def extract_strict_underlying_data(
  ...
  is_warrant = (
      ...
      or bool(re.search(r"(\.WS|[\.\/\-\s]WS|[Ww]{1,2})$", clean_sym))
  )

  is_fund_type = asset_sub in [
      "ETF",
      "ETN",
      "MUTUAL_FUND",
      "CEF",
  ...
  desc_pooled_match = bool(
      re.search(
          r"\b(ETF|ETN|FUND|TRUST|INDEX|PORTFOLIO|SHARES|NOTE|ETRACS|HOLDINGS|COMMODITY|CURRENCY|GOLD|SILVER|AGRICULTURE|BULL|BEAR|2X|3X|LEVERAGED|INVERSE|TREASURY|BOND)\b",
          desc,
      )
  )
  ...
  is_foreign_adr = (
      ...
      or bool(
          re.search(
              r"\b(CHINA|ISRAEL|UNITED"
              r" KINGDOM|BERMUDA|NETHERLANDS|GERMANY|SWITZERLAND|FRANCE|IRELAND|JAPAN|TAIWAN|BRAZIL|CANADA|KOREA|SOUTH"
              r" AFRICA)\b",
              desc,
          )
      )
  ...
  if is_halted_or_unquoted:
    liq_state, book_reason, book_note = (
        "UNVERIFIED_EMPTY_ORDER_BOOK",
        "asset_halted_or_unquoted",
        "EMPTY_ORDER_BOOK_ASSET_HALTED",
    )
  elif bid_s == 0.0 and ask_s == 0.0:
    liq_state = "UNVERIFIED_EMPTY_ORDER_BOOK"
    if tot_vol is not None and tot_vol > 0.0:
      book_reason, book_note = (
          "zero_bid_ask_depth_with_reported_volume",
          "ZERO_BID_ASK_DEPTH_REPORTED",
      )
    else:
      book_reason, book_note = (
          "zero_book_activity_recorded",
          "EMPTY_ORDER_BOOK_NO_VOLUME",
      )
...
def extract_in_memory_option_chains(
  ...
  def _parse_chain_payload(payload: Dict[str, Any]) -> None:
    ...
    for book_key, default_indicator in [
        ("callExpDateMap", "CALL"),
        ("putExpDateMap", "PUT"),
    ]:
