...
def extract_strict_underlying_data(
    ...
    desc_pooled_match = bool(
        re.search(
            r"\b(ETF|ETN|FUND|TRUST|INDEX|PORTFOLIO|SHARES|NOTE|ETRACS|HOLDINGS|COMMODITY|CURRENCY|GOLD|SILVER|AGRICULTURE|BULL|BEAR|2X|3X|LEVERAGED|INVERSE|TREASURY|BOND|IDX|ADMIRAL|INSTL|INVESTOR|TECH)\b",
            desc,
        )
    )
    ...
    is_foreign_country = bool(country) and bool(re.match(r"^[A-Z]{2,3}$", country)) and (country not in ["US", "USA"])
